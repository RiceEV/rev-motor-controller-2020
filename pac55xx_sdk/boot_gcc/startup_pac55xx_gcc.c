...
// @retval None
//=============================================================================
__attribute__((noinline)) void pac_delay_asm(uint32_t counts)
{
    __ASM
    (
        "       push {r1}               \n\t"
        "       movs r1, #1             \n\t"
        "delay_loop1:                   \n\t"
        "       sub  r0,r0,r1           \n\t"   // Decrement r0=counts by r1=decrement value
        "       cmp r0, #0              \n\t"
        "       bne delay_loop1         \n\t"   // Keep looping until counts == 0
        "       pop {r1}                \n\t"
    );
}
...
// @retval None
//=============================================================================
PAC5XXX_RAMFUNC __attribute__((noinline)) void pac_delay_asm_ramfunc(uint32_t counts)
{
    __ASM
    (
        "       push {r1}               \n\t"
