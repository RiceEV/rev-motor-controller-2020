/****************************************************************************
 * @file     isr_stubs.c
 * @brief    Interrupt Service Routine empty stubs
 * @date     21 September 2017
 *
 * @note
 * Copyright (C) 2017-2019, Qorvo, Inc.
 *
 * THIS SOFTWARE IS SUBJECT TO A SOURCE CODE LICENSE AGREEMENT WHICH PROVIDES,
 * AMONG OTHER THINGS:  (i) THAT IT CAN BE USED ONLY TO ADAPT THE LICENSEE'S
 * APPLICATION TO PAC PROCESSORS SUPPLIED BY QORVO, INC.;
 * (ii) THAT  IT IS PROVIDED "AS IS" WITHOUT WARRANTY;  (iii) THAT
 * QORVO, INC. IS NOT LIABLE FOR ANY INDIRECT DAMAGES OR FOR DIRECT
 * DAMAGES EXCEEDING US$1,500;  AND (iv) THAT IT CAN BE DISCLOSED TO AND USED
 * ONLY BY CERTAIN AUTHORIZED PERSONS.
 ******************************************************************************/
#define INCLUDE_EXTERNS
#include "bldc_common.h"

void MemCtl_IRQHandler(void)
{

}
void Wdt_IRQHandler(void)
{

}

void RTC_IRQHandler(void)
{

}

void ADC1_IRQHandler(void)
{

}

void ADC2_IRQHandler(void)
{

}

void ADC3_IRQHandler(void)
{

}

void QEPA_IRQHandler(void)
{

}

void QEPB_IRQHandler(void)
{

}

void QEPC_IRQHandler(void)
{

}

void QEPD_IRQHandler(void)
{

}

void GpioB_IRQHandler(void)
{

}

void GpioC_IRQHandler(void)
{

}

void GpioD_IRQHandler(void)
{

}

void GpioE_IRQHandler(void)
{

}

void GpioF_IRQHandler(void)
{

}

void GpioG_IRQHandler(void)
{

}

void I2C_IRQHandler(void)
{

}

void USARTA_IRQHandler(void)
{

}

void USARTB_IRQHandler(void)
{

}
  
void USARTC_IRQHandler(void) 
{
}



void USARTD_IRQHandler(void)
{

}

void CAN_IRQHandler(void)
{

}

void GPTimerA_IRQHandler(void)
{

}

void GPTimerB_IRQHandler(void)
{

}

void CCS_IRQHandler(void)
{

}

//ARM Cortex M4 System Service Routines
void NMI_Handler(void)
{

}
void HardFault_Handler(void)
{

}
void MemManageFault_Handler(void)
{

}
void BusFault_Handler(void)
{

}
void UsageFault_Handler(void)
{

}
void SVC_Handler(void)
{

}
void DebugMon_Handler(void)
{

}
void PendSV_Handler(void)
{

}


